...
const Utils = {
  ...
  // Replace last_updated:<readable Central time> in place; append only if missing.
  replaceLastUpdatedStamp: function (note, date = new Date()) {
    const stamp = `last_updated:${Utils.formatLastUpdated(date)}`;
    const existing = note || '';
    if (/\blast_updated:[^\n]*/.test(existing)) {
  ...
  stampAccountLastUpdated: async function (account) {
    ...
    await Utils.setAccountNote(account, Utils.replaceLastUpdatedStamp(existing));
  },
